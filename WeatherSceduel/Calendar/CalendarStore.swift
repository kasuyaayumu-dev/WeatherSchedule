...
final class CalendarStore {
  ...
  func condition(for date: Date) -> WeatherCondition? {
    guard let forecast = forecasts[calendar.startOfDay(for: date)] else { return nil }
    return WeatherCondition.from(rainProbability: forecast.aiPrediction.rainProbabilityPercent)
  }
